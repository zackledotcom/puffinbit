...
import { Activity, Zap, Thermometer, MemoryStick, Cpu, AlertTriangle, CheckCircle, RefreshCw } from 'phosphor-react'
...
export default function ModelHealthMonitor({ 
  ...
  className,
  onRefresh
}: ModelHealthMonitorProps) {  const [health, setHealth] = useState<ModelHealth>({
    overall: 87,
    performance: 92,
    memory: 78,
    temperature: 65,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setHealth(prev => ({
        ...prev,
        overall: Math.max(60, Math.min(100, prev.overall + (Math.random() - 0.5) * 4)),
        performance: Math.max(60, Math.min(100, prev.performance + (Math.random() - 0.5) * 6)),
        ...
        tokensPerSecond: Math.max(8, Math.min(25, prev.tokensPerSecond + (Math.random() - 0.5) * 2)),
      }))
    }, 3000)
    ...
    <TooltipProvider>
      <Card className={`glass-panel border-0 ${className}`}>
        <CardHeader className="pb-3">
          ...
          </div>          <Separator className="my-3 bg-gray-700" />
