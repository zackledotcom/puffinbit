...
export const useChatService = () => {
  ...
  const searchContext = useCallback(async (query: string): Promise<{ success: boolean; results: any[] }> => {
    try {
...
export const useMemoryService = () => {
  ...
  const loadMemoryStore = useCallback(async (): Promise<MemoryStore | null> => {
    setLoading(true)
    try {
  ...
  const addSummary = useCallback(async (summary: MemorySummary): Promise<boolean> => {
    try {
  ...
  const searchMemory = useCallback(async (query: string, limit?: number): Promise<{ success: boolean; results: any[] }> => {
    try {
  ...
  const createMemorySummary = useCallback(async (messages: ChatMessage[]): Promise<any> => {
    try {
  ...
  const enrichWithMemory = useCallback(async (prompt: string, options?: any): Promise<any> => {
    try {
  ...
  const updateSettings = useCallback(async (enabled: boolean, retentionDays?: number): Promise<boolean> => {
    try {
